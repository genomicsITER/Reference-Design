...
my $fqFile1, $fqFile2;
$fqFile1 = $inDataDir.$sample."_1.fq.gz";#"_R1_001.fastq.gz"  #"_1.fq.gz";
$fqFile2 = $inDataDir.$sample."_2.fq.gz";#"_R2_001.fastq.gz" #"_2.fq.gz";

# Pipeline executables and its directories
### ENTER THE CORRECT PATH TO THE FOLLOWING 3 VARIABLES ###
my $broadDir = '/mnt/nvme0n1/Broad_Ref_Pipeline_approved_package/broad';
my $QueueBroadBestPracticesDir = '/mnt/nvme0n1/Broad_Ref_Pipeline_approved_package/Queue_Broad_BestPractices';
my $homosapiensrefgenomeDir = '/mnt/nvme0n1/Broad_Ref_Pipeline_approved_package/homosapiensrefgenome';
# TOOLS
...
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "java -Dsamjdk.try_use_intel_deflater=true -jar $picard SortSam I=$bwamem_samFile O=$sort_bamFile SO=coordinate CREATE_INDEX=true TMP_DIR=$tmpDir";
Stop_Profiling();
sleep(60);
...
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "java -Dsamjdk.try_use_intel_deflater=true -jar $picard MarkDuplicates I=$sort_bamFile O=$bamDupRemFile M=$duplicateMetricsFile CREATE_INDEX=true TMP_DIR=$tmpDir";
Stop_Profiling();
sleep(60);
...
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "java -jar $gatk -T RealignerTargetCreator -nt $numThreads -R $refgenomeFastaFile -o $realnInterval -known:indels,vcf $dbSNPindel -I $bamDupRemFile";
Stop_Profiling();
sleep(60);
...
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "java -Djava.io.tmpdir=$tmpDir -jar $gatk_queue -R $refgenomeFastaFile -I $finalBam -l DEBUG -S $nfs_ExampleHaplotypeCaller -run -jobRunner CMPShell";
Stop_Profiling();
system("rm -rf $queueDir; rm -r $jobReport; rm -rf $ExampleDir");
sleep(60);
