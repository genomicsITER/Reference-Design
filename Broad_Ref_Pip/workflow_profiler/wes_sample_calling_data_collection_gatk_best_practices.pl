...
my $fqFile1, $fqFile2;
$fqFile1 = $inDataDir.$sample."_R1_001.fastq.gz"; #"_1.fq";
$fqFile2 = $inDataDir.$sample."_R2_001.fastq.gz"; #"_2.fq";
...
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "java -Xmx4g -jar $gatk -T BaseRecalibrator -I $bamRealignFile -R $refgenomeFastaFile -knownSites:mask,vcf $dbSNPvcf -o $recalOut -L $exome_targets_intervals -ip 50";
Stop_Profiling();
sleep(60);
...
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "java -Xmx8g -jar $gatk -T HaplotypeCaller -R $refgenomeFastaFile -I $finalBam -o $HCvcf -ERC GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -L $exome_targets_intervals -ip 50";
Stop_Profiling();
sleep(60);
