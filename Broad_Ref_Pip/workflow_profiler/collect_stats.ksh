...
usage() {
	echo "Usage: collect_stats.ksh <--sar || --free || --iostat || --collectl || --mpstat || --netstat || --pacct || --proc> <option list>"
...
mpstat_param_check() { #added-ben
	sar_delay_check
	steady_state_check
	users_check
}
free_param_check() { #added-ben
	sar_delay_check
	steady_state_check
	users_check
}
netstat_param_check() { #added-ben 1/6
	sar_delay_check
	steady_state_check
	users_check
}
...
create_collectl_vars() {
	COLLECTL_OUT=${PATH_PREFIX}.${SAR_INTERVAL}s.collectl
	echo "collectl out: [${COLLECTL_OUT}]"
	COLLECTL_ITERATIONS=$(((SLEEP+STEADY_STATE)*60/SAR_INTERVAL))
}
create_mpstat_vars() { #added-ben
...
start_collection() {
	...
	if [ -n "${KILL_EVERYTHING}" ] #added-ben 1/6
	then
...
collect_collectl() {
    updTS &&
    echo "Starting collectl collection...[${COLLECTL_OUT}] at ${MY_TS}" &&
    collectl -i ${SAR_INTERVAL} -scdmnCDMNL -f ${COLLECTL_OUT} &&
    #collectl -i ${SAR_INTERVAL} --import lustreClient.ph,s -scdmnCDMN -f ${COLLECTL_OUT} &&
    echo "collectl collection complete at ${MY_TS}" &
}
collect_pacct() {
