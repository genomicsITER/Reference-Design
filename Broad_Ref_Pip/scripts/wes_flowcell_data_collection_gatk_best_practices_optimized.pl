...
my $broadDir = '/cluster_share/Omics/Broad_Ref_Pipeline/broad';
my $bwaDir = "$broadDir/bwa";
my $bwa = "$bwaDir/bwa";
my $gatkDir = "$broadDir/gatk-protected/target";
my $gatk = "$gatkDir/GenomeAnalysisTK.jar";
my $gatk_queue = "/cluster_share/Omics/Broad_Ref_Pipeline/Queue_Broad_BestPractices/Queue.jar";
...
my $refgenomeFastaFile = '/cluster_share/Omics/data/ohsu/homosapiensrefgenome/Homo_sapiens_assembly19.fasta';
my $refgenomeBwtFile = '/cluster_share/Omics/data/ohsu/homosapiensrefgenome/Homo_sapiens_assembly19.fasta.bwt';
my $dbSNPvcf = '/cluster_share/Omics/Broad_Ref_Pipeline/Homo_sapiens_assembly19.dbsnp.vcf';
my $dbSNPindel = '/cluster_share/Omics/Broad_Ref_Pipeline/Homo_sapiens_assembly19.known_indels.vcf';
my $exome_targets_intervals = '/cluster_share/Omics/illumina/nexterarapidcapture_exome_uniqueintervals.bed';
my $nfs_ExampleIndelRealigner = "/mnt/app_hdd/scratch/aprabh2/Broad_Ref_Pipeline/Queue_Broad_BestPractices/ExampleIndelRealigner.scala";
my $nfs_ExampleBaseRecalibrator = "/mnt/app_hdd/scratch/aprabh2/Broad_Ref_Pipeline/Queue_Broad_BestPractices/ExampleBaseRecalibrator.scala";
my $nfs_ExamplePrintReads = "/mnt/app_hdd/scratch/aprabh2/Broad_Ref_Pipeline/Queue_Broad_BestPractices/ExamplePrintReads.scala";
my $nfs_ExampleHaplotypeCaller = "/mnt/app_hdd/scratch/aprabh2/Broad_Ref_Pipeline/Queue_Broad_BestPractices/ExampleHaplotypeCaller.scala";
my $cwd = `cwd`;
print "$cwd\n";
...
$fqFile1 = $inDataDir.$sample."_R1_001.fastq.gz"; #"_1.fastq.gz"; #"_1.fastq";
$fqFile2 = $inDataDir.$sample."_R2_001.fastq.gz"; #"_1.fastq.gz; #"_2.fastq";
...
my $readGroupHeader = "\@RG\\tID:$baseNameLane\\tLB:$baseName\\tSM:EXOME_TEST\\tPL:ILLUMINA";
...
=begin comment
my $stage_tag=SamtoolsView;
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "$samtools view -@ $numThreads -bt $genomeImportFile -S $bwamem_samFile -o $samview_unsortedFile";
Stop_Profiling();
sleep(60);

my $stage_tag=SamtoolsSort;
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "$samtools sort -@ $numThreads -l 5 $samview_unsortedFile $noext";
Stop_Profiling();
sleep(60);

run_and_log "$samtools index $samindex_bamFile";
sleep(10);
=end comment
=cut
...
#old MD Command
#run_and_log "java -Dsamjdk.try_use_intel_deflater=true -Xmx8g -jar $picard MarkDuplicates I=$mergedBam O=$bamDupRemFile M=$duplicateMetricsFile TMP_DIR=$tmpDir";
my $stage_tag=MarkDuplicates;
Start_profiling($stage_tag);
print "$stage_tag\n";
run_and_log "java -Dsamjdk.try_use_intel_deflater=true -Xmx8g -jar $picard MarkDuplicates I=$sort_bamFile O=$bamDupRemFile M=$duplicateMetricsFile CREATE_INDEX=true TMP_DIR=$tmpDir";
Stop_Profiling();
sleep(60);
...
#run_and_log "$samtools index $finalBam";
#sleep(10);
